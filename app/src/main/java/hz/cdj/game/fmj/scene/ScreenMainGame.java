...
public class ScreenMainGame extends BaseScreen {
	...
	/** 离队角色缓存，避免重新 createActor 时属性/装备被重置 */
	private static Map<Integer, Player> sDepartedActors = new HashMap<Integer, Player>();
...
//		update(0);
	...
	public void drawScene(Canvas canvas) {
		// 清除底部地图区域外残留画面（地图高 MAP_PIXEL_HEIGHT，底部到 SCREEN_HEIGHT 需清空）
	...
	public void createActor(int actorId, int x, int y) {
		// 优先从离队缓存恢复，避免属性/装备被重置
		if (sDepartedActors.containsKey(actorId)) {
			mPlayer = sDepartedActors.remove(actorId);
		} else {
	...
	public void deleteActor(int actorId) {
		for (int i = 0; i < sPlayerList.size(); i++) {
			if (sPlayerList.get(i).getIndex() == actorId) {
				// 缓存离队角色，保留等级/装备/属性
				sDepartedActors.put(actorId, sPlayerList.get(i));
