...
public class Combat extends BaseScreen implements CombatUI.CallBack {
	...
	/** 玩家角色中心坐标（按 256x160 画布等比缩放）*/
	public static final Point[] sPlayerPos = new Point[]{
		new Point(Global.csx(76), Global.csy(70)), new Point(Global.csx(108), Global.csy(66)), new Point(Global.csx(140), Global.csy(58))
	};
	...
	private void createBackgroundBitmap(int scrb, int scrl, int scrr) {
		mBackground = Bitmap.createBitmap(Global.SCREEN_WIDTH, Global.MAP_PIXEL_HEIGHT, Config.ARGB_8888);
		Canvas canvas = new Canvas(mBackground);
		canvas.drawColor(Global.COLOR_BLACK);

		// 原始战斗背景尺寸 160x96，拉伸到 256x160 全屏
		Rect dstFull = new Rect(0, 0, Global.SCREEN_WIDTH, Global.MAP_PIXEL_HEIGHT);
		ResImage img;
		img = (ResImage)DatLib.GetRes(DatLib.RES_PIC, 4, scrb);
		if (img != null) {
			Bitmap bmp = img.getBitmap(0);
			if (bmp != null) canvas.drawBitmap(bmp, null, dstFull, null);
		}
		img = (ResImage)DatLib.GetRes(DatLib.RES_PIC, 4, scrl);
		if (img != null) {
			Bitmap bmp = img.getBitmap(0);
			if (bmp != null) {
				Rect dst = new Rect(0, Global.MAP_PIXEL_HEIGHT - Global.csy(bmp.getHeight()),
						Global.csx(bmp.getWidth()), Global.MAP_PIXEL_HEIGHT);
		...
		if (img != null) {
			Bitmap bmp = img.getBitmap(0);
			if (bmp != null) {
				int w = Global.csx(bmp.getWidth());
				int h = Global.csy(bmp.getHeight());
				Rect dst = new Rect(Global.SCREEN_WIDTH - w, 0, Global.SCREEN_WIDTH, h);
				canvas.drawBitmap(bmp, null, dst, null);
			}
		}
		...
		case PerformAction:
			if (!mActionExecutor.update(delta)) { // 动作执行完毕
				if (isAllMonsterDead()) { // 怪物全挂
					...
					// 百倍金钱
					int actualMoney = Global.sCheatExpGold ? mWinMoney * 100 : mWinMoney;
					Player.sMoney += actualMoney;
...
//								p.setMP(p.getMaxMP()); CombatSuccess 中设置
				...
				int itemMult = Global.sCheatExpGold ? 100 : 1;
				for (Monster m : mMonsterList) {
					BaseGoods g = m.getDropGoods();
					if (g != null && sRandom.nextInt(101) < ppt) { //  ppt%掉率
						int dropNum = g.getGoodsNum() * itemMult;
						gm.addGoods(g.getType(), g.getIndex(), dropNum);
						Player.sGoodsList.addGoods(g.getType(), g.getIndex(), dropNum); // 添加到物品链表
					}
				}
					gl.addAll(gm.getGoodsList());
					gl.addAll(gm.getEquipList());
					mCombatSuccess = new CombatSuccess(actualExp, actualMoney, gl, lvuplist); // 显示玩家的收获
				} else { // 还有怪物存活
