...
public class Player extends FightingCharacter implements Externalizable {
	...
	public void setData(byte[] buf, int offset) {
		...
		setMagicLearnNum((int)buf[offset + 9] & 0xff);
	...
	public void drawState(Canvas canvas, int page) {
		// 合并双页为单页 — 画布 256×170，足够显示全部 10 行
		canvas.drawLine(37, 4, 37, 152, Util.sBlackPaint);
		// 第一列：基础属性
		TextRender.drawText(canvas, "等级   " + getLevel(), 41, 4);
		TextRender.drawText(canvas, "生命   " + getHP() + "/" + getMaxHP(), 41, 20);
		TextRender.drawText(canvas, "真气   " + getMP() + "/" + getMaxMP(), 41, 36);
		TextRender.drawText(canvas, "攻击力 " + getAttack(), 41, 52);
		TextRender.drawText(canvas, "防御力 " + getDefend(), 41, 68);
		// 第二列：扩展属性
		TextRender.drawText(canvas, "经验值", 41, 84);
		int w = Util.drawSmallNum(canvas, getCurrentExp(), 97, 84);
		TextRender.drawText(canvas, "/", 97 + w + 2, 84);
		Util.drawSmallNum(canvas, getLevelupChain().getNextLevelExp(getLevel()), 97 + w + 9, 90);
		TextRender.drawText(canvas, "身法   " + getSpeed(), 41, 104);
		TextRender.drawText(canvas, "灵力   " + getLingli(), 41, 120);
		TextRender.drawText(canvas, "幸运   " + getLuck(), 41, 136);
		...
		TextRender.drawText(canvas, sb.toString(), 41, 152);
	}
	...
	public void readExternal(ObjectInput in) throws IOException,
		...
		// 旧存档恢复：v2.2 及更早版本存的是共享链的 learnNum，
		// 可能被其他角色污染为 0。若魔法为 0 但等级 > 1，从等级链反推。
		// 注意：getLearnMagicNum(level) 在 Lv.36+ 返回 0（原始数据如此），需向后回溯。
		if (getMagicLearnNum() == 0 && getMagicChain() != null && getLevel() > 1) {
			int recovered = 0;
			if (mLevelupChain != null) {
				// 从当前等级向后回溯，找最后一个非零 learnMagicNum
				for (int lv = getLevel(); lv >= 1 && recovered == 0; lv--) {
					recovered = mLevelupChain.getLearnMagicNum(lv);
				}
			}
			// 若回溯失败，尝试从魔法链共享实例读取
			if (recovered == 0) {
				recovered = getMagicChain().getLearnNum();
			}
			if (recovered > 0) {
				setMagicLearnNum(recovered);
			}
		}
	}
