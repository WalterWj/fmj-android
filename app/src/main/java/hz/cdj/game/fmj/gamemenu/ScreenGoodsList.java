...
public class ScreenGoodsList extends BaseScreen {
	...
	private static final int ITEM_NUM = 6; // 界面上显示的条目数（原4，画布扩至256×170后增加到6）
	...
	public ScreenGoodsList(List<BaseGoods> list, OnItemSelectedListener l, Mode m) {
		...
		if (sbmpBg == null) {
			int W = Global.SCREEN_WIDTH, H = Global.MAP_PIXEL_HEIGHT;
			sbmpBg = Bitmap.createBitmap(W, H, Config.ARGB_8888);
			float[] pts = {40,21, 40,H-1, 40,H-1, 0,H-1, 0,H-1, 0,5, 0,5, 5,0, 5,0, 39,0,
					39,0, 58,19, 38,0, 57,19, 57,19, W-20,19, 41,20, W-20,20, 41,21, W-1,21,
					54,0, W-20,0, 40,H-1, W-1,H-1, 40,57, W,57, 40,58, W-20,58, 40,59, W-1,59,
					41,20, 41,H-1, 42,20, 42,H-1, W-1,21, W-1,57, W-1,59, W-1,H};
			...
			mRectGoodsDsp = new Rect(44, 61, W - 4, H - 2);
		}
	}
