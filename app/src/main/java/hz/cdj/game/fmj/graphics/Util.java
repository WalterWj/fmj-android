...
public class Util {
	...
	public static void init() {
		...
		if (bmpSideFrame == null) {
			bmpSideFrame = Bitmap.createBitmap(8, Global.MAP_PIXEL_HEIGHT, Bitmap.Config.ARGB_8888);
			canvas.setBitmap(bmpSideFrame);
			canvas.drawColor(Global.COLOR_WHITE);
			paint.setColor(Global.COLOR_BLACK);
			for (int i = 0; i < 8; i += 2) {
				canvas.drawLine(i, 0, i, Global.MAP_PIXEL_HEIGHT, paint);
			}
		}
	...
	public static void drawSideFrame(Canvas canvas) {
		...
		canvas.drawBitmap(bmpSideFrame, Global.MAP_RIGHT_OFFSET, 0, null);
	}
