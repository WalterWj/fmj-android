...
public class CombatUI extends BaseScreen {
	...
	/**
	 * 将 ResImage 从原始坐标系(160×96)缩放绘制到新坐标系(256×160)
	 */
	private static void drawScaledResImage(Canvas canvas, ResImage img, int frame, int origX, int origY) {
		Bitmap bmp = img.getBitmapRaw(frame - 1);
		if (bmp == null) return;
		int sx = Global.csx(origX);
		int sy = Global.csy(origY);
		int sw = Global.csx(origX + img.getWidth()) - sx;
		int sh = Global.csy(origY + img.getHeight()) - sy;
		if (sw <= 0 || sh <= 0) return;
	...
	private static final Point[] sPlayerIndicatorPos = new Point[]{
		new Point(Global.csx(69), Global.csy(45)), new Point(Global.csx(101), Global.csy(41)), new Point(Global.csx(133), Global.csy(33))
	};
	
	private static final Point[] sMonsterIndicatorPos = new Point[]{
		new Point(Global.csx(16), Global.csy(14)), new Point(Global.csx(48), Global.csy(3)), new Point(Global.csx(86), Global.csy(0))
	};
	...
	private class MainMenu extends BaseScreen {
		...
		@Override
		public void draw(Canvas canvas) {
			// 缩放绘制菜单图标（原位置 7, 96-iconH）
			drawScaledResImage(canvas, mMenuIcon, mCurIconIndex, 7, 96 - mMenuIcon.getHeight());
			int bgY = Global.csy(66);
			// 缩放绘制HP/MP信息背景
			drawScaledResImage(canvas, mPlayerInfoBg, 1, 49, 66);
			Player p = CombatUI.this.mPlayerList.get(mCurPlayerIndex);
			// 缩放绘制头像（原位置 50, 63）
			drawScaledResImage(canvas, mHeadsImg[p.getIndex() - 1], 1, 50, 63);
			if (p != null) {
				Util.drawSmallNum(canvas, p.getHP(), Global.csx(79), bgY + Global.csy(6)); // hp
				Util.drawSmallNum(canvas, p.getMaxHP(), Global.csx(104), bgY + Global.csy(6)); // maxhp
				Util.drawSmallNum(canvas, p.getMP(), Global.csx(79), bgY + Global.csy(17)); // mp
				Util.drawSmallNum(canvas, p.getMaxMP(), Global.csx(104), bgY + Global.csy(17)); // maxmp
			}
		...
		public void onKeyUp(int key) {
			...
			case 2://魔法技能
				Player magicPlayer = mPlayerList.get(mCurPlayerIndex);
				if (magicPlayer.getMagicChain() == null || magicPlayer.getMagicLearnNum() == 0) {
					Util.showMessage("未学会任何魔法!", 1000);
					break;
				}
			GameView.getInstance().pushScreen(new ScreenMagic(magicPlayer.getMagicChain(), magicPlayer.getMagicLearnNum(),
	...
	private class MenuMisc extends BaseScreen {
		...
		private class MenuState extends BaseScreen {
			...
			@Override
			public void draw(Canvas canvas) {
				int x = (Global.SCREEN_WIDTH - mBg.getWidth()) / 2;
				int y = (Global.MAP_PIXEL_HEIGHT - mBg.getHeight()) / 2;
