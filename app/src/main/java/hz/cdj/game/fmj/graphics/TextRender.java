...
public class TextRender {
	...
	public static void init() {
		...
		try {
			if (mHZKBuf == null) {
				in = GameView.getInstance().getGameContext().getAssets().open("HZK16");
				...
				mHZKBuf = baos.toByteArray();
				in.close();
			}
			
			if (mASCBuf == null) {
				in = GameView.getInstance().getGameContext().getAssets().open("ASC16");
	...
	public static int drawText(Canvas canvas, String text, Rect r, int y) {
		...
		for (; y <= r.top - 16 && i < buf.length; y += 16) {
			for (int x = 0; x < Global.SCREEN_WIDTH && i < buf.length;) {
