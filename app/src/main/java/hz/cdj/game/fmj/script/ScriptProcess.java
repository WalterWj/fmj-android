...
//			System.out.println("cmd_say");
		...
		public Operate getOperate(final byte[] code, final int start) {
			return new Operate() {
				...
				RectF rWithPic = new RectF(9, 50, Global.SCREEN_WIDTH - 9, Global.MAP_PIXEL_HEIGHT - 0.5f); // 有图边框
				Rect rWithTextT = new Rect(44, 58, Global.SCREEN_WIDTH - 15, 75); // 上
				Rect rWithTextB = new Rect(14, 76, Global.SCREEN_WIDTH - 15, Global.MAP_PIXEL_HEIGHT - 3); // 下
				RectF rWithoutPic = new RectF(9, 55, Global.SCREEN_WIDTH - 9, Global.MAP_PIXEL_HEIGHT - 0.5f); // 无图边框
				Rect rWithoutTextT = new Rect(14, 58, Global.SCREEN_WIDTH - 15, 75); // 上
				Rect rWithoutTextB = new Rect(14, 76, Global.SCREEN_WIDTH - 15, Global.MAP_PIXEL_HEIGHT - 3); // 下
...
//			System.out.println("cmd_movie");
		...
		public Operate getOperate(final byte[] code, final int start) {
			...
			public void draw(Canvas canvas) {
				...
				// 缩放到 256×170 全屏，匹配画布尺寸
				canvas.save();
				canvas.scale(Global.SX, Global.SY);
		...
		public Operate getOperate(final byte[] code, final int start) {
			return new Operate() {
				...
				
				{
					...
					bgx = (Global.SCREEN_WIDTH - bg.getWidth()) / 2;
					bgy = (Global.MAP_PIXEL_HEIGHT - bg.getHeight()) / 2;
				}
...
//			System.out.println("cmd_gaingoods");
		...
		public Operate getOperate(final byte[] code, final int start) {
			return new Operate() {
				...
				@Override
				public boolean process() {
					int num = Global.sCheatExpGold ? 100 : 1;
					goods.setGoodsNum(num);
					Player.sGoodsList.addGoods(goods.getType(), goods.getIndex(), num);
					time = 0;
		...
		public Operate getOperate(final byte[] code, final int start) {
			return new Operate() {
				...
				
				{
					...
					curY = imgBottom != null ? Global.MAP_PIXEL_HEIGHT - imgBottom.getHeight() : Global.MAP_PIXEL_HEIGHT;
					rect = new Rect(0,
							imgTop != null ? imgTop.getHeight() : 0,
							Global.SCREEN_WIDTH, curY);
				}
				...
				public void draw(Canvas canvas) {
					...
					if (imgTop != null) {
						imgBottom.draw(canvas, 1, 0, Global.MAP_PIXEL_HEIGHT - imgBottom.getHeight());
					}
				}
			};
		}
