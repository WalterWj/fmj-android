...
public class ScreenMenu extends BaseScreen {
	...
	private int mMenuX, mMenuY; // 菜单图绘制位置
	private int mSelectorOffX, mSelectorOffY; // SRS光标偏移量（相对于原160x106）
	...
	public ScreenMenu() {
		...
		mMenuX = (Global.SCREEN_WIDTH - mImgMenu.getWidth()) / 2;
		mMenuY = (Global.MAP_PIXEL_HEIGHT - mImgMenu.getHeight()) / 2;
		// SRS坐标是原始160x106画布的绝对坐标，需要加上偏移对齐
		mSelectorOffX = mMenuX - (160 - mImgMenu.getWidth()) / 2;
		mSelectorOffY = mMenuY - (96 - mImgMenu.getHeight()) / 2;
	}
	...
	public void draw(Canvas canvas) {
		...
		mImgMenu.draw(canvas, 1, mMenuX, mMenuY);
		mSrsSelector[mCurSelect].draw(canvas, mSelectorOffX, mSelectorOffY);
	}
